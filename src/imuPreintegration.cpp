...
public:
  ...
  void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg) {
    ...
    Eigen::Isometry3d imuOdomAffineIncre =
    ...
    if (lidarFrame != baselinkFrame) {
      ...
      tf2::Stamped<tf2::Transform> tb(tCur * lidar2Baselink,
    ...
    if (imuTime - last_path_time > 0.1) {
      ...
      while (!imuPath.poses.empty() &&
             stamp2Sec(imuPath.poses.front().header.stamp) <
                 lidarOdomTime - 1.0)
  ...
  void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg) {
    ...
    if (systemInitialized == false) {
      ...
      while (!imuQueOpt.empty()) {
        if (stamp2Sec(imuQueOpt.front().header.stamp) <
    ...
    double lastImuQT = -1;
    while (!imuQueImu.empty() && stamp2Sec(imuQueImu.front().header.stamp) <
                                     currentCorrectionTime - delta_t) {
